...
#==============================================================================
#
def sfiup(mu, nlr, dtau, ssa, xk, mug, wg, Ig05):
    '''
    Task:
...
#==============================================================================
#
def sfidn(mu, nlr, dtau, ssa, xk, mug, wg, Ig05):
    '''
    Task:
...
#
    if nit == 0: print("WARNING: nit = 0 -> MS = double scattering")   
#
    if phasefun == 'a':
        print("Aerosol:")
        xk_all = np.loadtxt(fname_xk_aer, skiprows=8)
        xk = xk_all[:, 0]
        Iup_MS_test = np.array([0.516133E-01, 0.487570E-01, 0.437619E-01, 0.383643E-01,
                              0.333712E-01, 0.290310E-01, 0.252454E-01, 0.216996E-01])
        Idn_MS_test = np.array([0.810312E-01, 0.905931E-01, 0.106710E+00, 
                                0.160098E+00, 0.239646E+00])
    else:
        print("Rayleigh:")
        xk = np.array([1.0, 0.0, 0.5]) # pure Rayleigh
        Iup_MS_test = np.array([0.128224E+00, 0.124847E+00, 0.119759E+00, 0.114495E+00,
                              0.109732E+00, 0.105696E+00, 0.102414E+00, 0.998377E-01])
        Idn_MS_test = np.array([0.935488E-01, 0.983778E-01, 0.950011E-01, 
                                0.911351E-01, 0.896627E-01])
#
    time_start = time.time()
#
#   Solve RTE @ Gauss nodes & all boundaries
    mug, wg, Igup, Igdn = gsit(nit, ng1, nlr, dtau, ssa, xk)
#   Compute intensity @ midpoint of every layer, both up & down 
...
#
    ...
    for imu, mu in enumerate(muup):
       Itoa = sfiup(mu, nlr, dtau, ssa, xk, mug, wg, Ig05)
       Itoa *= scalef
       Itest = Iup_MS_test[imu]
       reldev = 100.0*np.abs(1.0 - Itoa/Itest)
       print("mu = %.1f,  I = %.4e,  Itest = %.5e,  |err, o/o|=%.2f"
             %(mu, Itoa, Itest, reldev))
    print("Down:")
    for imu, mu in enumerate(mudn):
       Iboa = sfidn(mu, nlr, dtau, ssa, xk, mug, wg, Ig05)
       Iboa *= scalef
       Itest = Idn_MS_test[imu]
       reldev = 100.0*np.abs(1.0 - Iboa/Itest)
       print("mu = %.1f,  I = %.4e,  Itest = %.5e,  |err, o/o|=%.2f"
             %(mu, Iboa, Itest, reldev))
#
    time_end = time.time()
#
