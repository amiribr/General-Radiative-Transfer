...
#==============================================================================
#
def sfiup(mu, mu0, nlr, dtau, ssa, xk, mug, wg, Ig05):
    '''
    Task:
...
#==============================================================================
#
def sfidn(mu, mu0, nlr, dtau, ssa, xk, mug, wg, Ig05):
    '''
    Task:
...
def sglsup(mu, mu0, azr, tau0, xk):
    '''
    Task:
	    To compute single scattering at TOA of a homogeneous atmosphere.
    In:
        mu     d        cos(vza_up) < 0
        mu0    d        cos(sza) > 0
...
def sglsdn(mu, mu0, azr, tau0, xk):
    '''
    Task:
	    To compute single scattering at BOA of a homogeneous atmosphere.
    In:
        mu     d        cos(vza_up) > 0
        mu0    d        cos(sza) > 0
        azr    d[naz]   relative azimuths in radians; naz = len(azr)
...
#
    ...
    fname_bmark = 'test_gsit_R&A_SS+MS0.txt'
#
...
#   
    ...
    for imu, mu in enumerate(muup):
        Ims_toa = sfiup(mu, mu0, nlr, dtau, ssa, 0.5*ssa*xk, mug, wg, Ig05)
        Iss_toa = sglsup(mu, mu0, azr, nlr*dtau, 0.5*ssa*xk)   
        Itoa[imu, :] = (Iss_toa + Ims_toa)*scalef
...
#
    ...
    for imu, mu in enumerate(mudn):
        Ims_boa = sfidn(mu, mu0, nlr, dtau, ssa, 0.5*ssa*xk, mug, wg, Ig05)
        Iss_boa = sglsdn(mu, mu0, azr, nlr*dtau, 0.5*ssa*xk)   
        Iboa[imu, :] = (Iss_boa + Ims_boa)*scalef
